...
def CelestialLabelCatalog():
    labels= []
    vv= Star(_('NCP'),None,None,None)
    vv.SetVector3(RaDecToVector(0.0,90.0))
    labels.append(vv)
    vv= Star(_('SCP'),None,None,None)
    vv.SetVector3(RaDecToVector(0.0,-90.0))
    labels.append(vv)
    return labels
    ...
    def DrawStars(self,width,height,graphcon):
        ...
        for star in (self.Stars+self.Planets):
            if (star.Vector2 != None):
                halfsize= int(self.ObsCon.Faintest-star.Vmag-1.0)
                if (halfsize < 0): halfsize= 0
                if (halfsize > 10): halfsize= 11
