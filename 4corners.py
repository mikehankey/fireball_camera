...
if 1 == 1:
   ...
   (az, el) = radec_to_azel(ra,dec,config['cam_lat'],config['cam_lon'],config['cam_alt'], caldate)
   print ("AZ/EL of 1,1:", az,el)
   if el < 10:
      el = 10
   
   dist = 80/math.tan(math.radians(el))
   p1, p2 = find_point(float(config['cam_lat']), float(config['cam_lon']), dist, math.radians(az))
