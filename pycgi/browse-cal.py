...
def main():
   ...
   if cmd is None:
      browse_cal(day, cam_num)  

def browse_cal (day, cam_num):
   rand_num = random.randint(1,10000)
   print("<script src=/pycgi/del_cal.js?" + str(rand_num) + "></script>")
   print ("<h1>Browse Calibration Files<h1>")
   cal_files = get_cal_files(day, cam_num)
   #print(cal_files)
   for cal_file in cal_files:
      print (cal_file + "<BR>")
      print ("<img src=" + cal_file + "><br>")
      print ("Actions: <a href=\"javascript:del_cal('" + cal_file + "', event)\" onclick=\"confirm('Confirm Delete File?')\">Delete File</a><br>")
