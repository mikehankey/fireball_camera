...
def view(file, show):
    ...
    parts = file.split("/") 
    fn = parts[-1]
    fdate, cam_num = fn.split("-")
    cam_num = cam_num.replace(".avi", "")

    out_file = file.replace(".avi", ".jpg")
    out_file = "/var/www/html/out/time_lapse/" + cam_num + "/" + fdate + ".jpg"
    #print (file)
    print ("FILE:",file)
    ...
    while True:
        ...
        if frame is None:
            ...
            el = out_file.split("/")
            temp = el[-1]
            temp2 = temp.replace(".jpg", str(1000+count) + "-" + cam_num + ".jpg")
            save_file = out_file.replace(temp, temp2)
            print (save_file)
