...
def stack_loop_old(stack_queue, file, thread_num, start_time):
   ...
   while frame != 'STOP':
      if thread_num == 1:
         ...
         frame = stack_queue2.get()
      elif thread_num == 3:
         frame = stack_queue3.get()
      elif thread_num == 4:
         frame = stack_queue4.get()
      elif thread_num == 5:
         frame = stack_queue5.get()
      ...
      if frame is not None and frame != 'STOP' and frame != 'SKIP':
         ...
         if mx > (mx_avg * 2):
            ...
            #_, thresh = cv2.threshold(image_diff, int(mx_avg), 255, cv2.THRESH_BINARY)
